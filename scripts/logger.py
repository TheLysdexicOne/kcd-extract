...
TIMED_LOG_FILE = LOG_DIR / f"kcd-extract_{timestamp}.log"
STATIC_LOG_FILE = LOG_DIR / "kcd-extract.log"

# Create logger
logger = logging.getLogger("kcd-extract")
logger.setLevel(logging.DEBUG)  # Set minimum log level

# Create rotating file handler for timestamped log file
unique_handler = RotatingFileHandler(TIMED_LOG_FILE, maxBytes=10*1024*1024, backupCount=5)
unique_handler.setLevel(logging.DEBUG)

# Create file handler for statically named log file
static_handler = logging.FileHandler(STATIC_LOG_FILE, mode='w')  # 'w' mode to start with an empty file each run
...
MAX_LOGS = 5  # Keep only the latest 5 logs
log_files = sorted(LOG_DIR.glob("kcd-extract_*.log"), key=lambda f: f.stat().st_mtime, reverse=True)
